  ...
  ({ course, onPress, onToggleBookmark, isBookmarked }: CourseCardProps) => {
    ...
    return (
      <AnimatedTouchableOpacity
        className="mb-5 overflow-hidden rounded-[28px] border border-border/50 bg-surface"
        style={[
          animatedStyle,
          {
            shadowColor: '#000',
      ...
      >
          ...
          <View className="absolute left-3 top-3 flex-row gap-2">
            {isCompleted ? (
              <View className="rounded-full bg-emerald-500 px-3 py-1.5">
                <Text className="text-[10px] font-bold tracking-[1.5px] text-white">
                  COMPLETED
                </Text>
              </View>
            ) : isEnrolled ? (
              ...
              <View className="rounded-full bg-white/90 px-3 py-1.5">
                <Text className="text-[10px] font-bold tracking-[1.5px] text-text">
                  NEW
                </Text>
              </View>
            )}
        ...
        <View className="p-5">
          ...
          <Text className="mb-2 text-[18px] font-bold leading-7 text-text" numberOfLines={2}>
            {course.title}
          </Text>

          <Text className="mb-5 text-sm leading-6 text-text-muted" numberOfLines={2}>
            Learn at your own pace with structured lessons, clear outcomes, and hands-on practice.
          </Text>

          <View className="mb-5 flex-row items-center justify-between rounded-2xl bg-border/20 px-4 py-3">
            <View className="flex-row items-center">
              <View className="relative">
                <Image
                  ...
                  cachePolicy="memory-disk"
                />
                <View className="absolute -bottom-0.5 -right-0.5 h-3.5 w-3.5 rounded-full border-2 border-white bg-emerald-500" />
              </View>

              <View className="ml-3">
                <Text className="text-sm font-bold text-text">{course.instructor.name}</Text>
            ...
            <View className="flex-row items-center rounded-full bg-amber-50 px-3 py-2">
              <Ionicons name="star" size={14} color="#F59E0B" />
              <Text className="ml-1 text-sm font-bold text-text">
                {course.rating.toFixed(1)}
              </Text>
              <Text className="ml-1 text-[10px] font-semibold text-text-muted">
          ...
          <View className="flex-row items-center justify-between border-t border-border/60 pt-4">
            <View>
              <Text className="text-[11px] font-semibold uppercase tracking-[1.6px] text-text-muted">
              ...
              {isEnrolled ? (
                <View className="mt-2 flex-row items-center">
                  ...
                  <Text className="ml-2 text-sm font-semibold text-text-muted">completed</Text>
                </View>
              ) : (
                <View className="mt-1 flex-row items-end">
                  <Text className="mr-1 pb-1 text-sm font-bold text-text-muted">$</Text>
                  <Text className="text-3xl font-black tracking-tight text-text">
                    {priceText.whole}
                  </Text>
                  <Text className="pb-1 text-sm font-bold text-text-muted">.{priceText.fraction}</Text>
                </View>
              )}
            </View>
            ...
            {isEnrolled ? (
              ...
              <View className="rounded-2xl bg-text px-4 py-3">
