...
import {
    trackUserAction,
    withSentrySpan,
} from "@/core/services/sentryPerformance";
...
const SYSTEM_PROMPT = `You are AITV Assistance, an expert learning assistant. 
...
const tools: any = [
  {
    ...
    function: {
      name: "get_courses",
      description:
        "Get the current list of tech courses available in the catalog.",
      parameters: {
        type: "object",
        properties: {
          filter: {
...
export const processUserMessage = async (userMessage: string) => {
  const { addMessage, appendMessageChunk, setTyping, messages } =
    useAIStore.getState();
  ...
  try {
    ...
    if (responseMessage.tool_calls) {
      ...
      const stream = await withSentrySpan(
        "ai-tool-response",
        "ai.groq.stream",
        () =>
          groq.chat.completions.create({
      ...
      const finalMsg = useAIStore
        .getState()
        .messages.find((m) => m.id === aiMessageId);
      if (finalMsg) {
        const { courses } = useCourseStore.getState();
        const mentionedCourse = courses.find((c) =>
          finalMsg.text.includes(c.title),
        );
        if (mentionedCourse) {
          useAIStore.setState((state) => ({
            messages: state.messages.map((m) =>
              m.id === aiMessageId ? { ...m, courseId: mentionedCourse.id } : m,
            ),
          }));
        }
      }
          ...
          messages: [
            {
              role: "system",
              content:
                'You are a course recommendation engine. Given a list of courses and a user intent, return a JSON object with a key "ids" containing an array of course IDs that best match the intent. ONLY return the JSON object.',
            },
            {
              role: "user",
