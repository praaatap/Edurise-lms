...
import { getRouteFromNotificationData, useRootLayoutController } from '@/shared/hooks/useRootLayoutController';
    ...
    <ThemeProvider value={isDark ? DarkTheme : DefaultTheme}>
      <View style={{ flex: 1, backgroundColor: bgColor }}>
        ...
        <InAppNotificationBanner
          ...
          onPress={() => {
            const route = getRouteFromNotificationData(foregroundNotification.data ?? {});
            if (route) {
              setPendingRoute(route);
            }
            setForegroundNotification((current) => ({ ...current, visible: false }));
          }}
          onDismiss={() => setForegroundNotification((current) => ({ ...current, visible: false }))}
        />
        <CustomDialog
