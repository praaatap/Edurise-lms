...
export default function BookmarksScreen() {
  ...
  const handleCoursePress = useCallback(
    (course: Course) => {
      clarityService.logEvent('course_viewed', {
        courseId: course.id,
        title: course.title,
        source: 'bookmarks',
      });
  ...
  const handleLongPress = useCallback((course: Course) => {
    ...
    analytics.logEvent('bookmarks_course_long_pressed', {
      courseId: course.id,
      title: course.title,
    });
  ...
  const handleAction = useCallback(
    async (action: 'delete' | 'share' | 'enroll') => {
      ...
      if (action === 'delete') {
        ...
        clarityService.logEvent('course_unbookmarked', {
          courseId: selectedCourse.id,
          source: 'bookmarks_screen',
        });
        analytics.logEvent('bookmarks_course_removed', {
          courseId: selectedCourse.id,
        ...
        clarityService.logEvent('course_shared', {
          courseId: selectedCourse.id,
        });
        analytics.logEvent('bookmarks_course_shared', {
          courseId: selectedCourse.id,
          title: selectedCourse.title,
        });
        await Share.share({
  ...
  if (bookmarkedCourses.length === 0) {
    return (
      <View
        className="flex-1 bg-background dark:bg-dark-bg items-center justify-center"
        style={{ paddingBottom: tabBarContentInset }}
      >
        <EmptyState
          icon={Bookmark}
          ...
          onAction={() => {
            analytics.logEvent('bookmarks_explore_tapped');
    ...
    <View className="flex-1 bg-background dark:bg-dark-bg">
      <LegendList
        ...
        ListHeaderComponent={
          <View className="px-1 mb-6">
            <Text className="text-3xl font-extrabold text-text dark:text-dark-text tracking-tighter">
              Saved Courses
            </Text>
            <Text className="text-base text-text-muted dark:text-dark-text-muted mt-1 font-medium">
              Long press a course for options
            </Text>
          </View>
        }
      />
      ...
      >
        <BottomSheetView className="px-6 py-4 pb-12">
          <Text
            className="text-xl font-extrabold text-text dark:text-dark-text mb-1"
            numberOfLines={1}
          >
