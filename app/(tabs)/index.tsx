...
import {
  WalkthroughStep,
  WalkthroughTooltip,
} from '@/shared/components/ui/WalkthroughTooltip';
...
const WALKTHROUGH_STEPS: WalkthroughStep[] = [
  {
    ...
    description:
      'Keep your daily learning streak alive by checking in and completing courses every single day!',
    icon: Flame,
    iconColor: '#F97316',
    targetKey: 'header',
    placement: 'bottom',
  },
  {
    ...
    description:
      'Easily navigate through Web Dev, Mobile, AI, Design, and Cloud courses with interactive pills.',
    icon: Compass,
    iconColor: '#3B82F6',
  ...
  },
  {
    ...
    description:
      'Stuck on a concept? Tap the green AI Tutor button to start a personalized learning chat at any time.',
    icon: Sparkles,
    iconColor: '#22C55E',
    targetKey: 'aiButton',
    placement: 'top',
...
function useHomeLogic() {
  ...
  const handleCoursePress = useCallback(
    (course: Course) => {
      ...
      analytics.logEvent('course_tapped', {
        courseId: course.id,
        title: course.title,
        source: 'home',
      });
  ...
  const inProgressCourses = useMemo(
    () =>
      courses.filter(
        (c) =>
          enrolledCourses.includes(c.id) && !completedCourses.includes(c.id),
      ),
    ...
    () => (
      <View className="mb-2" onLayout={measureElements}>
        ...
        <View
          ref={headerRef}
          collapsable={false}
          className="mb-6 mt-4 flex-row justify-between items-center px-4"
        >
          <View>
          ...
          <View className="flex-row items-center gap-3">
            {streak > 0 && (
              <View className="flex-row items-center bg-orange-50 dark:bg-orange-950/60 px-3 py-1.5 rounded-full border border-orange-100 dark:border-orange-900/50">
                ...
                <Text className="ml-1.5 text-sm font-black text-orange-600">
                  {streak}
                </Text>
              </View>
            )}
            <TouchableOpacity className="w-12 h-12 bg-secondary dark:bg-secondary-dark rounded-full items-center justify-center">
          ...
          <View className="mb-8">
            ...
            >
              {inProgressCourses.map((course) => (
                ...
                >
                  <View
                    className="bg-white dark:bg-dark-surface rounded-[32px] p-4 border border-border/40 dark:border-dark-border shadow-sm"
                  ...
                  >
                    ...
                    />
                    <Text
                      className="text-sm font-bold text-text dark:text-dark-text mb-2"
                      numberOfLines={1}
                    >
                      {course.title}
                    </Text>
                    ...
                    <Text className="text-[10px] text-primary font-bold mt-2 uppercase tracking-widest">
                      Resume Learning
                    </Text>
                  </View>
                </TouchableOpacity>
            ...
            >
              {uniqueInstructors.map((inst, idx) => (
                <TouchableOpacity
                  ...
                  onPress={() => {
                    analytics.logEvent('home_instructor_tapped', {
                      instructorId: inst.id,
                      name: inst.name,
                    });
                    router.push(`/instructor/${inst.id}` as Href);
                  }}
                  activeOpacity={0.8}
                >
                  <View
                  ...
                  >
                    <Image
                      source={
                        inst.avatar ||
                        `https://i.pravatar.cc/400?img=${(idx + 1) * 3}`
                      }
                  ...
                  <Text className="text-text dark:text-dark-text font-extrabold text-sm mt-4">
                    {inst.name}
                  </Text>
        ...
        {aiRecommendedIds.length > 0 && (
          <View className="mb-8">
            ...
            >
              {courses
                .filter((c) => aiRecommendedIds.includes(c.id))
                .map((course) => (
      ...
      {error && !isLoading && courses.length === 0 && (
        <View className="flex-1 items-center justify-center px-8">
          ...
          <TouchableOpacity
            ...
            onPress={() => {
              analytics.logEvent('home_error_retry');
              refreshCourses();
            }}
            activeOpacity={0.8}
          >
